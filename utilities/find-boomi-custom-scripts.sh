...
find "$PROCESSES_FOLDER" -name "*.xml" -type f | while read -r xml_file; do
    ...
    if grep -qE 'dataprocessscript.*language="(groovy|javascript)' "$xml_file" 2>/dev/null; then

        # Extract the component ID from <Id> tag
        process_id=$(grep -o '<Id>[^<]*</Id>' "$xml_file" | head -1 | sed 's/<Id>//;s/<\/Id>//')

        # Extract the component Name from <Name> tag
        # Escape any commas in the name for CSV safety
        process_name=$(grep -o '<Name>[^<]*</Name>' "$xml_file" | head -1 | sed 's/<Name>//;s/<\/Name>//' | sed 's/,/;/g')
        ...
        # Use perl to extract each shape with dataprocessscript and its attributes
        ...
        ' | while IFS=$'\t' read -r shape_name script_language; do
            ...
            echo "${process_id},\"${process_name}\",\"${shape_name}\",\"${script_language}\",\"${folder}\"" >> "$temp_file"
        done
    fi
done

# Print CSV header
echo "process_id,process_name,shape_name,script_language,folder"

# Deduplicate by process_id+shape_name (columns 1 and 3) and output sorted results
if [ -f "$temp_file" ] && [ -s "$temp_file" ]; then
    sort -t',' -k1,1 -k3,3 -u "$temp_file"
fi
